...
/**
 * Plugin implementation of the 'edit_in_place_field_stock' formatter.
 *
 * @FieldFormatter(
 *   id = "edit_in_place_field_stock",
...
class EditPlaceStockFormatter extends StringFormatter implements ContainerFactoryPluginInterface
{
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if (!empty($items) ) {
      ...
      foreach($items as $delta => $item) {
        ...
        $stock = array( "value" => $stocks);
        $view_value = $this->viewValue($item);
        $field_values[] = $stock;

        $stock = array_column($field_values, 'value');
        foreach ($stock as $key => $value) {
          $stock[$key] = intval($value);
        }

        $render_elements[] = [
          '#theme' => 'edit_in_place_stock_values',
    ...
    $elements[0] = [
      'field_container' => [
        ...
        'form_container' =>  $this->formBuilder->getForm('Drupal\edit_in_place_field\Form\EditPlaceStockForm', [
          'values' => $field_values,
